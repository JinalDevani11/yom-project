<?php 
  ...
  {
    $id=$_GET['id'];
    $rec="select * from `offer` where id=".$id;
  ...
  if(isset($_POST['submit'])){
      ...
      $title=$_POST['title'];
      $des=$_POST['des'];
       $check_query = "SELECT * FROM `offer` WHERE title='$title'";
      $res1=mysqli_query($con, $check_query);
      if(isset($_GET['id']))
      {
         $id = $_GET['id'];
        if(mysqli_num_rows($res1) == 0){
           $sql="UPDATE `offer` SET icon='$icon',title='$title',des='$des' where id=".$_GET['id'];
        ...
        }else
        {
         ...
         if(mysqli_num_rows($res1) == 0){
             $sql="INSERT INTO `offer`(`icon`,`title`,`des`) VALUES ('$icon','$title','$des')";
            mysqli_query($con, $sql);
            header("location:view_offer.php");   
          }else
         {
